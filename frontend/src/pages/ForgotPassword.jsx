...
const ForgotPassword = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err?.data?.message || err.error || 'Failed to dispatch reset email.');
    }
  };
    ...
    <div className="min-h-screen flex text-gray-200">
      ...
      <div className="relative w-full lg:w-1/2 flex items-center justify-center bg-[#0B0E14] p-8">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
          <button onClick={() => navigate('/login')} className="flex items-center text-gray-400 hover:text-white transition-colors mb-8 text-sm bg-transparent border-0 cursor-pointer">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button type="submit" disabled={isLoading} className="w-full btn-primary py-3 flex justify-center items-center">
              {isLoading ? <><FaSpinner className="animate-spin mr-2" /> Sending link...</> : 'Send Reset Link'}
            </button>
          </form>
        </motion.div>
      </div>
